...
/*
  ...
  - دعم فتح وثيقة الإتقان فقط بعد اكتمال 180 يومًا
*/
...
function normalizeJourney() {
  ...
  return RAW_MONTHS.map((month, monthIndex) => {
    ...
    return {
      ...
      // نترك عدد الأيام الفعلية داخل المحتوى كما هو حتى لا ينكسر فتح الأسابيع والأيام.
      // أما إجمالي الرحلة المعتمد في المنصة والوثيقة فهو 180 يومًا عبر COURSE_TOTALS.
...
export const COURSE_TOTALS = {
  ...
  // تعديل مهم:
  // إجمالي الرحلة المعتمد: 6 أشهر × 30 يوم = 180 يوم.
  days: safeNumber(
    learningJourneyMeta?.days,
    courseMap.reduce((total, month) => total + safeNumber(month.totalDays), 0) || 180
  ),

  questions: safeNumber(
    learningJourneyMeta?.questions,
    courseMap.reduce((total, month) => total + safeNumber(month.totalQuestions), 0) || 540
  ),
  gradedQuestions: safeNumber(learningJourneyMeta?.gradedQuestions, 540),

  totalMonths: safeNumber(learningJourneyMeta?.months, courseMap.length || 6),
  totalWeeks: safeNumber(learningJourneyMeta?.weeks, 24),

  // تعديل مهم:
  totalDays: safeNumber(learningJourneyMeta?.days, 180),

  totalQuestions: safeNumber(learningJourneyMeta?.questions, 540),
  totalGradedQuestions: safeNumber(learningJourneyMeta?.gradedQuestions, 540),

  // إضافات صريحة تمنع رجوع الحسابات القديمة إلى 28 يومًا للشهر.
  daysPerMonth: 30,
  daysPerWeek: 7,
};
...
export function calculateProgress(completedDayIds = []) {
  ...
  // تعديل مهم:
  // لا نرجع إلى 168. المعتمد الآن 180.
...
export function calculateMonthProgress(monthId, completedDayIds = []) {
  ...
  // نعرض الشهر كـ 30 يومًا حتى لا يظهر 28 في الواجهة.
  // لكن اكتمال الشهر منطقيًا يبقى مبنيًا على أيام المحتوى الموجودة فعليًا حتى لا ينكسر فتح الشهر التالي.
  const contentDays = safeArray(month.days);
  const totalDays = COURSE_TOTALS.daysPerMonth || 30;
  const completedDays = contentDays.filter((day) => completedSet.has(day.id)).length;
  const percentage = totalDays > 0 ? Math.round((completedDays / totalDays) * 100) : 0;
  ...
  return {
    ...
    completedDays,
    percentage: Math.min(Math.max(percentage, 0), 100),
    isCompleted: contentDays.length > 0 && completedDays >= contentDays.length,
  };
}
